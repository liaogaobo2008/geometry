...
namespace Riches.Visio
{
    namespace Geometry
    {
        ...
        public class TextArea : IShape
        {
            ...
            private Font _font = new Font("微软雅黑", 12f);
            ...
            private void RefreshData()
            {
                ...
                this._area = new Rectangle((int)this.X, (int)this.Y, s.Width + 4, s.Height+20);
                this.IsDirty = false;
            }
            public void Invalid(Graphics g)
            {
        ...
        public class Dot : IShape
        {
            ...
            //public void Scale( Point center)
            //{
            //    float mx = this.X - center.X;
            //    float my = center.Y - this.Y;
            //    mx = mx * this.Canvas.ScaleRation;
            //    my = my * this.Canvas.ScaleRation;
            ...
            public void Scale(float ration, Point center)
            {
                float mx = this.X - center.X;
                float my = center.Y - this.Y;
                mx = mx * ration;
            ...
            //private Point _orgPoint = Point.Empty;
            ...
            //    set
            //    {
            //        if (value != this._orgPoint)
            //        {
            //            if (_orgPoint.IsEmpty)
            //            {
            //                this.X = value.X;
            //                this.Y = value.Y;
            //            }
            ...
            public void Delete()
            {
             ...
             //   foreach(var l in this.Lines)
               ...
               var rems= this.Canvas.Shapes.Where(t =>
                {
                    if (t.ShapeType == ShapeType.RemAngle)
                    {
                        var remangle = t as RemAngle;
                        if (remangle.Dot.Id == this.Id)
                            return true;
                    }
                    return false;
                }).ToList();

                foreach (RemAngle angle in rems)
                {
                    angle.Delete();
                }

                foreach (FillArea fillArea in this.Canvas.Shapes.Where(s => s.ShapeType == ShapeType.FillArea).ToList())
                {
           ...
           // public List<RemAngle> RemAngles { get; set; } = new List<RemAngle>();
            ...
            public void Invalid(Graphics g)
            {
                ...
                else if (this.IsActive)
                {
                    ...
                    rect.Inflate(2, 2);
                    using (var pen = new SolidBrush(this.Color))
                        g.FillEllipse(pen, this.Area);
                ...
                {
                    using (var pen = new SolidBrush(this.Color))
                        if(this.Area.X>=0&&this.Area.Y>=0)
                        g.FillEllipse(pen, this.Area);
                }

                if (this.Canvas.IsDisplayXYCoord)
                {
                    ...
                    {
                        g.DrawString(String.Format(this.Text + "(x={0:N2},y={1:N2})", this.X, this.Y), Font, Brushes.Green, TextPos);
                    }
                }
                else
            ...
            public string SerializeObject()
            {
              ...
              //  sbd.Append("\"P\":{\"X\":" + this.X + ",\"Y\":" + this.Y + "},");
                ...
                sbd.Append("\"Color\":\"" + this.Color.Name + "\",");
                sbd.Append("\"Width\":" + this.Width + ",");
              //  sbd.Append("\"OrgPoint\":\"" + this.OrgPoint.X+","+this.OrgPoint.Y + "\",");
            ...
            private void Attched(IShape[] shapes, Point location)
            {
                ...
                foreach (var d in shapes)
                {
                    if (d.ShapeType == ShapeType.Dot)
                    {
                        ...
                        foreach (RemAngle sh in this.Canvas.Shapes.Where(t => t.ShapeType == ShapeType.RemAngle && t.Id == destDot.Id))
                        {
                            sh.Dot = this;
                        }
        ...
        public class Line : IShape
        {
            ...
            {
                using (var pen = new Pen(Color.DarkOrchid, 1f))
                {
            ...
            public void Invalid(Graphics g)
            {
                ...
                {//线段被选中
                    ...
                    using (var pen = new Pen(this.Color, this.Width))//ActiveColor
                    {
                        pen.DashStyle = DashStyle;
                ...
                g.DrawString(this.Text, font, Brushes.Black,this.MidPoint);
                this.IsDirty = false;
            }
        ...
        public class Circle : IShape
        {
            ...
            public void Invalid(Graphics g)
            {
                ...
                {
                    using (var pen = new Pen(this.Color, this.Width + 2))
                    using (var pen2 = new Pen(Color.DarkRed, 1))
                    {
                        RectangleF rect = new RectangleF() { X = this.Center.X - Diam / 2, Y = this.Center.Y - Diam / 2, Width = this.Diam, Height = this.Diam };
                        var rect0 = rect;
                        var inc = this.Width + 2;
                        rect0.Inflate(inc, inc);
                        var rect1 = rect;
                        rect1.Inflate(-inc, -inc);

                        g.DrawArc(pen2, rect0, 0f, 360f);

                        g.DrawArc(pen, rect, 0f, 360f);

                        g.DrawArc(pen2, rect1, 0f, 360f);
                    }
                }
                else if(this.Diam>1)
                {
        ...
        public class Parabolic : IShape
        {
                    ...
                    while (Regex.Matches(str, "[a-zA-Z]").Count > 0)
                    {
                        ...
                        for (int i = 0; i < str.Length; i++)
                        {
                            ...
                            if (char.IsLetter(c))
                            {
                                ...
                                object dval = dt.Compute(strVal, "");
                                res = Convert.ToSingle(dval==DBNull.Value?0:dval);
            ...
            private string ParasePow(string str,DataTable dt, Stack<char> stack)
            {
                int idx = 0; double res = 0f;
            ...
            public void Invalid(Graphics g)
            {
                ...
                int expWidth= TextRenderer.MeasureText(Express, this.Font).Width;
                if (txtPos.X + expWidth > this.CanvasCtl.Width)
                    txtPos.X = this.CanvasCtl.Width - expWidth-20;

                string powStr=this.Express.Replace("**", "?");
                if (powStr.Contains('?'))
                {
            ...
            private Font _font = new Font("微软雅黑", 12f, FontStyle.Regular);
